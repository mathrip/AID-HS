...
def calibrate_smoothing(coords, faces, start_v=5000, n_iter=20, cortex_mask=None, label='label-hipp'):
...
def smooth_array(input_array, neighbours,n_iter=70,cortex_mask=None):
    ...
    import copy
    neighbours_new = copy.deepcopy(neighbours)
    old_array = input_array
    if cortex_mask is not None:
        for v,n in enumerate(neighbours_new):
            n=np.array(n)
            neighbours_new[v]=n[cortex_mask[n]]
    else: 
        cortex_mask = np.ones(len(neighbours_new),dtype=bool)
    for iteration in np.arange(n_iter):
        new_array = np.zeros_like(old_array)
        #this bit is quite loopy and could potentially be sped up
        vertices=np.arange(len(neighbours_new))[cortex_mask]
        for v in vertices:
            n=neighbours_new[v]
            new_array[v] = np.mean(old_array[np.hstack([n,v])],axis=0)
        old_array = new_array
